...
package body Screen is
	...
	begin
		...
		My_Screen.Initialize (External_VCC => False);
		My_Screen.Initialize_Layer (Layer => 1,
										Mode => M_1);

		My_Screen.Turn_On;
		My_Screen.Hidden_Buffer (Layer => 1).Set_Source (Current_Background_Color);
		My_Screen.Hidden_Buffer (Layer => 1).Fill;
		My_Screen.Update_Layer (Layer => 1);
	...
	begin
		Bitmapped_Drawing.Draw_Char
		  (My_Screen.Hidden_Buffer (Layer => 1).all,
	  Start      => (X, Y),
	  Char       => Msg,
	  Font       => Current_Font,
